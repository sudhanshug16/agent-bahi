...
async function sourceView(session: BusinessSession, tenantId: string, taxCaseId: string, sourceId?: string): Promise<Record<string, unknown>[]> {
  ...
  return Promise.all(rows.rows.map(async (row) => {
    ...
    const readiness = await sourceReadinessInSession(session, tenantId, taxCaseId, String(row.id));
    return {
      sourceId: String(row.id), sourceKind: String(row.source_kind), sourcePeriod: row.source_period == null ? null : String(row.source_period), sourceAsOf: row.source_as_of == null ? null : String(row.source_as_of), parser: { identity: String(row.parser_identity), version: String(row.parser_version), status: String(row.parser_status) }, status: String(readiness.readinessStatus), readinessStatus: String(readiness.readinessStatus), nextAction: readiness.readinessStatus === "READY" ? "Source is human-confirmed and currently matches exact artifact and fact bindings." : readiness.readinessStatus === "STALE" ? "Prepare a reassessment after reviewing changed artifact or fact bindings." : readiness.readinessStatus === "REJECTED" ? "Prepare a new assessment after addressing the human rejection." : NEXT_SOURCE_ACTION, createdAt: String(row.created_at), artifact: { linkId: String(row.link_id), artifactId: String(row.artifact_id), contentHash: String(row.content_hash), byteSize: Number(row.byte_size), mediaType: String(row.media_type), originalFilename: String(row.original_filename), createdAt: String(row.artifact_created_at), deduplicated: links.rows.length > 1, linkCount: links.rows.length },
    };
  }));
}
