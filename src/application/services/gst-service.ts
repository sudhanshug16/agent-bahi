...
export async function executePartyGstProfileCreate(sessionRunner: BusinessSessionRunner, envelope: GstEnvelope<PartyGstProfileCreatePayload>): Promise<CommandResult<PartyGstProfileCreateResult>> {
  ...
  const normalizedState = stateCode(envelope.payload.stateCode, "stateCode");
  if (treatment === "REGISTERED" && gstin!.slice(0, 2) !== normalizedState) throw new DomainError("GSTIN_STATE_MISMATCH", "registered party GSTIN must match its state code");
  assertRange(envelope.payload.effectiveFrom, envelope.payload.effectiveTo); const status = envelope.payload.status ?? "ACTIVE";
...
export interface GstPostingPlan { sellerRegistrationId: string; buyerProfileId: string; sellerGstin: string; sellerStateCode: string; buyerGstin?: string; buyerTreatment: GstPartyTreatment; buyerStateCode: string; geometry: GstGeometry; taxableMinor: number; taxMinor: number; grossMinor: number; localComponent?: GstLocalComponent; itcTreatment?: ItcTreatment; riskFlags: string[]; evidenceIds: string[]; components: GstTaxComponentPlan[]; }
...
function factsFor(block: GstDocumentBlock, lines: InputLine[]): Map<number, GstLineFact> {
  const raw = block.lineFacts ?? block.lines ?? [];
  const sourceLines = new Set<number>();
  for (const line of lines) {
    if (!Number.isSafeInteger(line.lineNumber) || line.lineNumber < 1 || sourceLines.has(line.lineNumber)) throw new DomainError("GST_LINE_FACT_REQUIRED", "document lines must have unique positive line numbers");
    sourceLines.add(line.lineNumber);
  }
  const map = new Map<number, GstLineFact>();
  raw.forEach((fact, index) => {
    if (!fact || typeof fact !== "object") throw new DomainError("GST_LINE_FACT_REQUIRED", "each GST line fact must be an object");
    const candidate = fact.lineNumber ?? (raw.length === lines.length ? index + 1 : undefined);
    if (typeof candidate !== "number" || !Number.isSafeInteger(candidate) || candidate < 1 || !sourceLines.has(candidate) || map.has(candidate)) throw new DomainError("GST_LINE_FACT_REQUIRED", "GST line facts must exactly cover each source line once");
    const n = candidate;
    map.set(n, fact);
  });
  if (map.size !== lines.length || lines.some((line) => !map.has(line.lineNumber))) throw new DomainError("GST_LINE_FACT_REQUIRED", "GST line facts must exactly cover each source line once");
  return map;
}
...
export async function prepareGstPosting(session: BusinessSession, args: { tenantId: TenantId; bookSetId: BookSetId; partyId: string; documentDate: string; documentType: "SALE" | "PURCHASE"; block: GstDocumentBlock; lines: InputLine[] }): Promise<GstPostingPlan> {
  ...
  const normalizedProfileGstin = profileGstin === undefined ? undefined : normalizeGstin(profileGstin, "partyGstin");
  const profileValue = profileId ?? normalizedProfileGstin;
  ...
  if (String(profile.treatment) === "REGISTERED") {
    const storedGstin = String(profile.gstin ?? "");
    if (storedGstin.slice(0, 2) !== String(profile.state_code)) throw new DomainError("GSTIN_STATE_MISMATCH", "registered party GSTIN must match its state code");
    if (normalizedProfileGstin !== undefined && storedGstin !== normalizedProfileGstin) throw new DomainError("GST_PARTY_PROFILE_MISMATCH", "selected party GSTIN does not match profile");
  }
  ...
  return { sellerRegistrationId: String(registration.id), buyerProfileId: String(profile.id), sellerGstin: String(registration.gstin), sellerStateCode: ownState, ...(profile.gstin == null ? {} : { buyerGstin: String(profile.gstin) }), buyerTreatment: String(profile.treatment) as GstPartyTreatment, buyerStateCode: otherState, geometry, taxableMinor, taxMinor, grossMinor: taxableMinor + taxMinor, ...(geometry === "INTRA_STATE" ? { localComponent: local } : {}), ...(itcTreatment ? { itcTreatment } : {}), riskFlags, evidenceIds: [...new Set(components.flatMap((c) => c.evidenceIds))].sort(), components };
}
...
export async function persistGstSnapshot(session: BusinessSession, args: { tenantId: TenantId; bookSetId: BookSetId; documentType: "SALE" | "PURCHASE"; documentId: string; plan: GstPostingPlan; now: string }): Promise<string> {
  const snapshotId = randomUUID(); await session.execute("INSERT INTO gst_tax_snapshots (id, tenant_id, book_set_id, document_type, sales_invoice_id, vendor_bill_id, seller_registration_id, buyer_profile_id, seller_gstin, seller_state_code, buyer_gstin, buyer_treatment, buyer_state_code, local_component, geometry, rounding_policy, taxable_minor, tax_minor, gross_minor, itc_treatment, risk_flags_json, evidence_json, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", [snapshotId, args.tenantId, args.bookSetId, args.documentType, args.documentType === "SALE" ? args.documentId : null, args.documentType === "PURCHASE" ? args.documentId : null, args.plan.sellerRegistrationId, args.plan.buyerProfileId, args.plan.sellerGstin, args.plan.sellerStateCode, args.plan.buyerGstin ?? null, args.plan.buyerTreatment, args.plan.buyerStateCode, args.plan.localComponent ?? null, args.plan.geometry, GST_V1_ROUNDING_POLICY, args.plan.taxableMinor, args.plan.taxMinor, args.plan.grossMinor, args.plan.itcTreatment ?? null, JSON.stringify(args.plan.riskFlags), JSON.stringify(args.plan.evidenceIds), args.now]);
