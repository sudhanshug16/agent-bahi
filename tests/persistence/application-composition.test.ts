...
test("production composition exposes typed services without raw persistence handles", async () => {
  ...
  try {
    ...
    expect(Object.keys(application).sort()).toEqual(["account", "bankMatch", "bankReconciliation", "bankStatement", "bill", "bookSet", "bookSetScope", "closePack", "company", "compliance", "expense", "fixedAssets", "invoice", "journal", "ledger", "party", "payroll", "receipt", "tax", "taxCase", "tenant", "vendorPayment"]);
