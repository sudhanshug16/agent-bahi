...
test("bootstrap applies the journal migration and scope resolution is active-only", async () => {
  ...
  try {
    ...
    expect(native.query("SELECT schema_version, last_migration_id FROM database_control").get()).toEqual({ schema_version: 8n, last_migration_id: "0013_fx_v1" });
    expect(native.query("SELECT hash FROM __drizzle_migrations ORDER BY created_at").all()).toHaveLength(5);
    native.close();
