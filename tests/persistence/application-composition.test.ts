...
test("production composition exposes typed services without raw persistence handles", async () => {
  ...
  try {
    ...
    expect(Object.keys(application).sort()).toEqual(["account", "bankMatch", "bankReconciliation", "bankStatement", "bill", "bookSet", "bookSetScope", "closePack", "company", "compliance", "expense", "fiscalYearRollover", "fixedAssets", "invoice", "journal", "ledger", "mca", "party", "payroll", "receipt", "sourceStaging", "tax", "taxCase", "tenant", "vendorPayment", "withholdingStatement", "zohoBackup"]);
