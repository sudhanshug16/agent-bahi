...
test("production composition exposes typed services without raw persistence handles", async () => {
  ...
  try {
    ...
    expect(Object.keys(application).sort()).toEqual(["account", "bankMatch", "bankReconciliation", "bankStatement", "bill", "bookSet", "bookSetScope", "closePack", "company", "compliance", "expense", "fixedAssets", "invoice", "journal", "ledger", "party", "payroll", "receipt", "tax", "tenant", "vendorPayment"]);
...
test("bootstrap applies the journal migration and scope resolution is active-only", async () => {
  ...
  try {
    ...
    expect(native.query("SELECT hash FROM __drizzle_migrations ORDER BY created_at").all()).toHaveLength(12);
    native.close();
