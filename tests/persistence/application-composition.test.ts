...
test("production composition exposes typed services without raw persistence handles", async () => {
  ...
  try {
    ...
    expect(Object.keys(application).sort()).toEqual(["account", "bankMatch", "bankReconciliation", "bankStatement", "bill", "bookSet", "bookSetScope", "company", "fixedAssets", "invoice", "journal", "ledger", "party", "payroll", "receipt", "tax", "tenant", "vendorPayment"]);
...
test("bootstrap applies the journal migration and scope resolution is active-only", async () => {
  ...
  try {
    ...
    expect(native.query("SELECT schema_version, last_migration_id FROM database_control").get()).toEqual({ schema_version: 8n, last_migration_id: "0014_payroll_v1" });
    expect(native.query("SELECT hash FROM __drizzle_migrations ORDER BY created_at").all()).toHaveLength(6);
    native.close();
