...
describe("Personal TaxCase foundation", () => {
  test("enforces eligibility, immutable membership versions, revision staleness, and shared transport parity", async () => {
    ...
    try {
      ...
      expect(() => native.query("INSERT INTO tax_case_memberships (id, tax_case_id, tenant_id, membership_version_id, version, book_set_id, ledger_revision, created_at) VALUES (?, ?, ?, ?, 2, ?, 0, ?)").run(randomUUID(), "case-1", individual.tenantId, version2.id, company.defaultBookSetId, "2026-08-23T00:00:00.000Z")).toThrow();
      ...
      expect(native.query("SELECT COUNT(*) AS count FROM idempotency_records WHERE tenant_id = ?").get(individual.tenantId)).toEqual({ count: 7n });
      native.close();
