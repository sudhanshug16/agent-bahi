...
describe("Zoho Books backup/CSV import V1", () => {
  ...
  it("previews and human-confirms a privacy-safe synthetic 44-file export without posting", async () => {
    ...
    expect(preview.files).toHaveLength(44); expect(preview.rows.staged).toBe(13); expect(preview.rows.unsupported).toBe(31); expect(preview.rows.rejected).toBe(0); expect(preview.postsJournal).toBe(false); expect(preview.warnings.some((warning) => warning.includes("DUPLICATE_HEADER"))).toBe(true);
