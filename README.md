...
- [Zoho Books and Frappe Books Feature Parity Matrix](docs/discovery/zoho-frappe-parity.md): Feature-by-feature comparison of Zoho Books (India) and Frappe Books against agent-bahi's design choices. Confirms that agent-bahi aligns with Zoho's workflow parity while enforcing stricter tenant isolation, immutable reversal lineage, and deterministic CLI behavior. Frappe Books is referenced as concept/behavior source only (AGPL-3.0; no code reuse). Validates coverage of core bookkeeping, GST, payroll, FX, fixed assets, banking, and compliance domains.
...
- [Statutory Compliance Baseline](docs/discovery/statutory-workflow-contracts.md): Contracts and requirements for TDS/TCS, annual income-tax returns, company statutory compliance (Companies Act 2013), and Registrar filings. Linked to separate verified research matrices: [TDS/TCS Compliance](docs/discovery/tds-tcs-compliance-matrix.md), [Annual Income-Tax Compliance](docs/discovery/annual-income-tax-compliance-matrix.md), [MCA Companies Act Compliance](docs/discovery/mca-companies-act-compliance-matrix.md). Marks unverified items as OPEN and ensures official-source linkage.
