...
- Source artifact IDs/hashes, source IDs, and parser versions as immutable provenance
...
TaxComputation derives from exactly one SEALED, CURRENT FilingSnapshot and never posts to the books. Multiple immutable ExportRuns are allowed; each inherits the snapshot's exact ID/hash and TaxCase-bound pack/rule/schema chain. A real immutable validation run with immutable validation-evidence rows must be SEALED and exactly `PASSED` for the same computation, output hash, output schema, and validator artifact before an ExportRun is created or selected. An immutable SubmissionBinding records one exact snapshot ID/hash plus export ID/hash and the exact `PASSED` validation outcome. Its current validity is represented only by append-only binding-state events; SubmissionAttempt has one composite FK to that immutable binding candidate and no FK to mutable binding status or a separately supplied export tuple. An attempt may be inserted only when the latest binding event is `ACTIVE`, the bound export and snapshot are `CURRENT`, and validation is exactly `PASSED`. `FAILED`, `INCOMPLETE`, `UNKNOWN`, `REVIEW`, `STALE`, and `DRIFTED` exports are structurally ineligible.
...
**Control:** Persist an immutable form-eligibility evaluation whose exact composite identity binds `selected_form` to the frozen eligibility fact-set ID/hash, its exact source-snapshot ID/hash, and the official predicate artifact ID/kind/hash. `selected_form` is not an independently writable nullable TaxCase assertion. A database guard requires zero unresolved predicates and the exact evaluation FK before `READY`, validation, computation, or export; missing or conflicting facts remain `REVIEW/BLOCK`.
...
The exact AuthorityPack tuple `(authority_pack_id, pack_content_hash, governing_act, period_key, filing_trigger, rule_snapshot_id, schema_artifact_id, schema_artifact_hash)` binds to each TaxCase and FilingSnapshot through the identically ordered candidate key; downstream exports inherit that exact tuple through the snapshot/computation chain. Missing or incompatible pack or schema component returns REVIEW/BLOCK.
...
`READY` and `DECLARED_NOT_APPLICABLE` are fail-closed states, not prose labels. `READY` requires a nonempty catalog snapshot in database state `SEALED`, immutable ordinal/`requirement_key` entries, one normalized readiness evidence-set entry for every required catalog entry, exact FKs from each entry to that catalog entry and its source artifact/hash, equal catalog/evidence counts, and a database-enforced `OPEN -> SEALED` evidence-set assertion. The READY source FK has identical seven-column child/target arity and order: tenant, case, source, evidence-set ID, catalog snapshot ID, catalog snapshot hash, and literal `SEALED` state. The evidence-entry FK prevents extras and its primary key prevents duplicates; the seal assertion checks both anti-joins so every catalog entry has exactly one evidence entry before `SEALED`. Sealed catalog/evidence parents and every child row are immutable under database guards. Catalog/evidence counts and hashes derive from complete ordered rows at seal; a caller-supplied hash cannot make a partial set complete. A TaxCase `READY` transition must use the exact composite FK to an evidence set whose `seal_state` is literally `SEALED`; a copied source status or summary hash cannot satisfy the gate. If role multiplicity is needed, it is a separate normalized child under one catalog entry. `DECLARED_NOT_APPLICABLE` requires a non-null exact source/evidence binding, actor, reason, and scope. A missing, duplicated, mismatched, or partial binding leaves the source unresolved and blocks the affected action.
...
**Control:** Before readiness evaluation, the complete required source catalog is deterministically enumerated from taxpayer facts, applicable BookSets, tax heads, the governing rule snapshot, and the selected official schema. Database triggers or equivalent guarded stored routines seal the catalog only when its nonempty entry count matches, reject catalog-entry writes after sealing, and seal an evidence set only when its exact count and two anti-join checks prove one-to-one coverage. SQLite uses an `IMMEDIATE` transaction and aborting triggers; PostgreSQL uses row-locking triggers and deferred constraint triggers where needed; MySQL/InnoDB uses a locking seal procedure plus write/state triggers because it has no deferred constraints. The READY FK includes the literal `SEALED` state, so an empty or not-yet-enumerated catalog, an internally consistent partial evidence set, a duplicate coverage row, a count/set mismatch, or an application-only status can never pass `READY`. Required unresolved states block only the affected computation, export, or filing action; they do not block unrelated BookSet work. Optional unresolved sources remain visible and do not silently disappear.
...
The product records evidence supplied by the taxpayer or CA; it does not infer a later portal state from elapsed time or from a local export. Internal states (`PREPARED`, `EXPORTED`, `UNKNOWN`) are never mistaken for external government status.
