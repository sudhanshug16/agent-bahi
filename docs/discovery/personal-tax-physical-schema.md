...
- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `fact_set_id`, `fact_set_hash` (computed over canonical fact payload/manifest), `sealed_at`, `sealed_by`. Nullable: none (all required for sealed fact provenance).
...
- **Immutability:** fact_set_id, canonical fact payload/manifest, fact_set_hash, sealed_at, and sealed_by are all immutable after creation. Fact sets are append-only; facts cannot be recomputed in place.
...
- **Contents:** Immutable frozen eligibility facts (determined form applicability, taxpayer status, income sources, filing obligations, entity classification) with exact evidence provenance. The fact-set hash explicitly includes the exact frozen fact-set ID/hash and all fact elements.
...
- **Internal states (PT-013):** preparation/readiness state is separate from external portal status. Internal filing lifecycle is exactly `PREPARED`, `EXPORTED`, or `UNKNOWN` (canonical local state enum per document's established casing); case readiness remains an internal product state. `validated` is an ExportRun validation outcome, not a filing lifecycle state; do not use `validated` as a TaxCase lifecycle value. Remove contradictory lifecycle use of `validated` without deleting validation gates that apply to export/submission.
...
- **Foreign keys:** composite FK `(tenant_id, tax_case_id, period_key, filing_sequence, tax_case_id)` to TaxCase candidate key `UNIQUE(tenant_id, pan, period_key, filing_sequence, tax_case_id)` ensuring exact applicability match including pan, sequence, and period (child FilingSnapshot tuple must exactly match declared TaxCase candidate key in columns/order); composite FK `(tenant_id, tax_case_id, eligibility_fact_set_id, eligibility_fact_set_hash)` to eligibility_fact_sets exact candidate key ensuring frozen facts match this case; composite FK `(authority_pack_id, authority_pack_content_hash, governing_act, period_key, filing_trigger, rule_snapshot_id)` to authority_packs exact candidate key.
...
- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `snapshot_id`, `evidence_artifact_id`, `evidence_artifact_hash`, `parser_version` (where parser applies to artifact type). Nullable: `source_id` (optional; if artifact is tied to external source).
- **Primary key:** `(tenant_id, tax_case_id, snapshot_id, evidence_artifact_id)`.
- **Unique keys:** one row per snapshot per artifact.
- **Foreign keys:** composite FK `(tenant_id, tax_case_id, snapshot_id)` to filing_snapshots; composite FK `(tenant_id, evidence_artifact_id, evidence_artifact_hash)` to evidence_artifacts candidate key UNIQUE(tenant_id, artifact_id, artifact_hash) with exact hash binding enforced by database relationship (no bypassing via null or mismatch); optional `(tenant_id, tax_case_id, source_id)` to external_sources.
- **Immutability:** all required fields immutable after snapshot seals. evidence_artifact_hash is NOT NULL and forms an exact composite FK; hash cannot be bypassed or null-substituted.
- **Gate:** Artifact membership deterministically enumerated at snapshot creation. Hash verifies artifact integrity across lifecycle via exact composite FK. Parser_version is required where applicable and immutable.

### 2.13 `tax_computations`

- **Fields:** NOT NULL: `computation_id`, `tenant_id`, `tax_case_id`, `snapshot_id`, `authority_pack_id`, `authority_pack_content_hash`, `governing_act`, `period_key`, `filing_trigger`, `rule_snapshot_id`, version, result_hash (computed over tax computation payload/result). Nullable: tax computation state.
...
- **Foreign keys:** composite FK `(tenant_id, tax_case_id, snapshot_id, authority_pack_id, authority_pack_content_hash, governing_act, period_key, filing_trigger, rule_snapshot_id)` to filing_snapshots exact candidate key ensuring snapshot integrity and applicability match.
- **Immutability:** computation ID, snapshot binding, applicability tuple, version, and result_hash are immutable. A computation never posts to the books.
- **Atomicity and Match:** Snapshot and TaxComputation are created in one atomic transaction. TaxComputation repeats the `version` field, computes result_hash, and captures the snapshot's exact applicability tuple. At creation, snapshot's declared `tax_computation_version` and `tax_computation_hash` must match the computation's version and result_hash, or the transaction rolls back.
...
- **Fields:** NOT NULL: `export_id`, `tenant_id`, `tax_case_id`, `snapshot_id`, `computation_id`, `computation_result_hash`, export format, export timestamp, content_hash, output_schema_version, validation_identity. Nullable: creation metadata non-identity fields.
...
- **Foreign keys:** `(tenant_id, tax_case_id)` to TaxCase (composite FK); composite FK `(tenant_id, tax_case_id, snapshot_id, computation_id, computation_result_hash)` to tax_computations exact candidate key ensuring export binds canonical computation with verified result hash.
- **Immutability:** export ID, snapshot binding, computation binding (ID and result_hash), and content hash are immutable. A new export is created for a snapshot when format or generation parameters change.
...
- **Gate:** multiple ExportRuns may exist per snapshot. Each export binds exactly one canonical computation from that snapshot (no mismatch possible via composite FK). Selection is recorded separately via `submission_bindings`. Composite FK `(tenant_id, tax_case_id, selected_export_id)` from submission_bindings ensures selected export belongs to same case.
...
- **Fields:** `tenant_id`, `tax_case_id`, `source_id`, source type, readiness status, source identity and period, optional `institution_code`, optional `account_identifier`, reconciliation state, and actor metadata.
...
- **Foreign keys:** composite FK `(tenant_id, tax_case_id)` to TaxCase; composite FK `(tenant_id, tax_case_id, source_id, book_set_id)` to external_source_booksets exact PK (ensures source/BookSet pair is valid and both are bound to the same case); composite FK `(tenant_id, evidence_artifact_id, evidence_artifact_hash)` to evidence_artifacts candidate key UNIQUE(tenant_id, artifact_id, artifact_hash). All FKs enforce same-tenant scope via tenant_id; no loose BookSet-only FK or null-field bypass can substitute for the source/BookSet junction or evidence artifact binding.
...
- **Gate:** reconciliation links books and evidence without replacing either; a required conflict cannot be marked ready by acknowledgement alone. Source/BookSet pairing must exist via external_source_booksets junction in the same case. All required fields (artifact ID/hash, outcome, amount, reason, actor, timestamp) must be present and NOT NULL; no nullable composite bypass possible.

### 2.21 `correction_metadata`

- **Fields:** NOT NULL: `tenant_id`, `predecessor_tax_case_id`, `successor_tax_case_id`, `successor_sequence` (from successor TaxCase), `successor_authority_pack_id`, `successor_authority_pack_content_hash`, verified_trigger (source of correction requirement), applicable_mechanism_id, `rule_reference_id` (immutable reference to rule_snapshots), `correction_evidence_artifact_id`, `correction_evidence_artifact_hash`, verifier, verification_timestamp. Nullable: deadline_source.
...
- **Foreign keys:** composite FK `(tenant_id, predecessor_tax_case_id, successor_tax_case_id)` to tax_case_lineage exact candidate key (binds correction metadata to explicit lineage relation, not to nonexistent `(tenant_id, tax_case_id)` entry); composite FK `(tenant_id, successor_authority_pack_id, successor_authority_pack_content_hash, applicable_mechanism_id, rule_reference_id, verified_trigger)` to authority_pack_correction_routes exact candidate key ensuring the successor pack contains the verified mechanism route; composite FK `(tenant_id, correction_evidence_artifact_id, correction_evidence_artifact_hash)` to evidence_artifacts candidate key UNIQUE(tenant_id, artifact_id, artifact_hash).
...
- **Fields:** NOT NULL: `tenant_id`, `authority_pack_id`, `pack_content_hash`, `applicable_mechanism_id`, `rule_reference_id`, `verified_trigger`, route metadata (mechanism description, deadline, severity). Nullable: none (all required for sealed correction route).
- **Primary key:** `(tenant_id, authority_pack_id, pack_content_hash, applicable_mechanism_id, rule_reference_id, verified_trigger)`.
- **Unique keys:** one route per pack per mechanism per rule per trigger; prevents duplicate routes.
- **Foreign keys:** composite FK `(authority_pack_id, pack_content_hash)` to authority_packs exact identity ensuring the route is bound to a specific sealed AuthorityPack and its exact content. Composite FK `(rule_reference_id)` to rule_snapshots ensuring rule reference is valid and sealed.
...
- **Candidate keys:** `UNIQUE(tenant_id, authority_pack_id, pack_content_hash, applicable_mechanism_id, rule_reference_id, verified_trigger)` — exact correction-route identity for FK targets from correction_metadata.
...
- **Fields:** `tenant_id`, `lineage_id`, `pan`, `period_key`, `predecessor_tax_case_id`, `predecessor_sequence`, `successor_tax_case_id`, `successor_sequence`, reason.
- **Primary key:** `(tenant_id, lineage_id)`.
- **Unique keys:** lineage identity within the tenant.
- **Candidate keys:** `UNIQUE(tenant_id, predecessor_tax_case_id, successor_tax_case_id)` — one-to-one immediate lineage; predecessor and successor appear in exactly one lineage row each. Also declare UNIQUE `(tenant_id, predecessor_tax_case_id)` and UNIQUE `(tenant_id, successor_tax_case_id)` to enforce one predecessor per successor and one successor per predecessor.
- **Foreign keys:** composite FK `(tenant_id, pan, period_key, predecessor_sequence, predecessor_tax_case_id)` to tax_cases exact candidate key; composite FK `(tenant_id, pan, period_key, successor_sequence, successor_tax_case_id)` to tax_cases exact candidate key. Both FKs enforce same tenant, shared taxpayer (pan), shared period, and sequence validation.
