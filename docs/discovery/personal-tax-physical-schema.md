...
- **Candidate keys:** `UNIQUE(tenant_id, pan, period_key, filing_sequence, tax_case_id)` — exact lineage tuple for composite FK targets in tax_case_lineage and correction_metadata. `UNIQUE(tenant_id, tax_case_id, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id)` — exact TaxCase/rule applicability tuple for eligibility_fact_sets and FilingSnapshot. `UNIQUE(tenant_id, tax_case_id, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id, authority_pack_id, authority_pack_content_hash)` — exact successor TaxCase applicability-and-pack tuple for correction_metadata.
...
- **Fields:** NOT NULL: `snapshot_id`, `tenant_id`, `tax_case_id`, `pan`, `period_key`, `filing_sequence`, `eligibility_fact_set_id`, `eligibility_fact_set_hash`, `authority_pack_id`, `authority_pack_content_hash`, `governing_act`, `filing_trigger`, `rule_snapshot_id`, declared `tax_computation_version`, declared `tax_computation_hash` (expected hash for canonical computation), `integrity_status` (`CURRENT`, `STALE`, or `DRIFTED`), as_of_instant timestamp, snapshot_content_hash (computed over inputs: ordered BookSet entries + artifact hashes + exact frozen fact-set ID/hash + computation version/hash). Nullable: `invalidation_reason`, `invalidation_source`, `invalidation_at`, and `superseding_tax_case_id` only while `integrity_status = CURRENT`.
...
- **Foreign keys:** composite FK `(tenant_id, tax_case_id, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id)` to TaxCase candidate key `UNIQUE(tenant_id, tax_case_id, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id)` ensuring the child matches the exact TaxCase/rule applicability tuple; composite FK `(tenant_id, tax_case_id, eligibility_fact_set_id, eligibility_fact_set_hash, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id)` to the eligibility_fact_sets candidate key `UNIQUE(tenant_id, tax_case_id, fact_set_id, fact_set_hash, pan, period_key, filing_sequence, governing_act, filing_trigger, rule_snapshot_id)` ensuring the frozen facts match this case, snapshot applicability, and rule snapshot; composite FK `(authority_pack_id, authority_pack_content_hash, governing_act, period_key, filing_trigger, rule_snapshot_id)` to authority_packs exact candidate key; nullable composite FK `(tenant_id, superseding_tax_case_id)` to TaxCase primary key, required whenever `integrity_status` is STALE or DRIFTED.
...
- **Integrity status and invalidation:** `integrity_status` has `CHECK (integrity_status IN ('CURRENT', 'STALE', 'DRIFTED'))`. While `CURRENT`, `invalidation_reason`, `invalidation_source`, `invalidation_at`, and `superseding_tax_case_id` are NULL. A `STALE` or `DRIFTED` snapshot must have all four fields non-null, and `superseding_tax_case_id` has an exact same-tenant FK to TaxCase. The status and invalidation fields are changed only by the atomic invalidation gate in §3; prior snapshot contents remain immutable.
...
- **Gate:** FilingSnapshot must bind exactly one sealed AuthorityPack via exact (ID, content_hash, Act, period, trigger, rule), bind exactly one sealed eligibility_fact_set via the exact (ID, hash, pan, period, sequence, Act, trigger, rule) tuple to the same case, and enumerate exact BookSet ledger versions via junction (not as-of alone). Every ExportRun is tied to exactly one FilingSnapshot via tenant-scoped composite FK, and only a `CURRENT` snapshot can feed computation, export, selection, or submission. Snapshot and its canonical TaxComputation created atomically; their version/hash must match or integrity violated. Child FilingSnapshot tuple must exactly match the TaxCase candidate key in columns/order, including pan, sequence, applicability, and rule snapshot; a stale/different-rule fact set cannot satisfy the database relationship.
...
- **Fields:** NOT NULL: `export_id`, `tenant_id`, `tax_case_id`, `snapshot_id`, `computation_id`, `computation_result_hash`, export format, export timestamp, content_hash, output_schema_version, validation_identity, `validation_outcome` (immutable export validation result: PASSED, FAILED, INCOMPLETE, UNKNOWN, or REVIEW; reflects deterministic schema/computation validation state, not lifecycle), `integrity_status` (`CURRENT`, `STALE`, or `DRIFTED`). Nullable: `invalidation_reason`, `invalidation_source`, `invalidation_at`, and `superseding_tax_case_id` only while `integrity_status = CURRENT`; creation metadata non-identity fields.
- **Primary key:** `export_id`.
- **Unique keys:** candidate key `UNIQUE(tenant_id, tax_case_id, export_id)` (enforces each export belongs to exactly one case). `UNIQUE(tenant_id, tax_case_id, export_id, content_hash, validation_outcome)` — immutable export identity and validation tuple for SubmissionAttempt. `UNIQUE(tenant_id, tax_case_id, export_id, content_hash, validation_outcome, integrity_status)` — exact export identity and current eligibility tuple for submission bindings. `(snapshot_id, export_format, export_timestamp, content_hash)` optional secondary uniqueness for content tracking.
- **Foreign keys:** `(tenant_id, tax_case_id)` to TaxCase (composite FK); composite FK `(tenant_id, tax_case_id, snapshot_id, computation_id, computation_result_hash)` to the exact TaxComputation candidate key `UNIQUE(tenant_id, tax_case_id, snapshot_id, computation_id, result_hash)`, ensuring export binds the canonical computation and its verified result hash; nullable composite FK `(tenant_id, superseding_tax_case_id)` to TaxCase primary key, required whenever `integrity_status` is STALE or DRIFTED. No additional snapshot identity fields are needed here: the referenced TaxComputation tuple is immutable and itself exact-FK-bound to the complete FilingSnapshot candidate key.
...
- **Integrity status and invalidation:** `integrity_status` has `CHECK (integrity_status IN ('CURRENT', 'STALE', 'DRIFTED'))`. While `CURRENT`, `invalidation_reason`, `invalidation_source`, `invalidation_at`, and `superseding_tax_case_id` are NULL. A `STALE` or `DRIFTED` export must have all four fields non-null, and `superseding_tax_case_id` has an exact same-tenant FK to TaxCase. The status and invalidation fields are changed only by the atomic invalidation gate in §3; export identity, output, and validation evidence remain immutable.
...
- **Gate:** multiple ExportRuns may exist per snapshot. Each export binds exactly one canonical computation from that snapshot (no mismatch possible via composite FK). Submission eligibility is the exact conjunction `validation_outcome = 'PASSED' AND integrity_status = 'CURRENT' AND snapshot.integrity_status = 'CURRENT'`; no other outcome or status can be selected, bound, or receive a submission attempt. Selection is recorded separately via `submission_bindings`, whose exact export tuple FK and conditional CHECK enforce this gate.

### 2.14a `submission_bindings`

- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `selected_export_id`, `selected_export_content_hash`, `selected_export_validation_outcome`, `selected_export_integrity_status`, `binding_status` (`ACTIVE`, `STALE`, or `DRIFTED`), `selected_at`, `selected_by`. Nullable: `locked_at` (null only before first attempt).
...
- **Candidate keys:** `(tenant_id, tax_case_id, locked_at)` — unique before first attempt to enforce single null-locked_at state; after first attempt locked_at is non-null and row becomes immutable. `(tenant_id, tax_case_id, locked_at, selected_export_id, selected_export_content_hash, selected_export_validation_outcome, selected_export_integrity_status, binding_status)` — exact locked selection and invalidation tuple for SubmissionAttempt. `(tenant_id, tax_case_id, selected_export_id, selected_export_content_hash, selected_export_validation_outcome, selected_export_integrity_status, binding_status)` — exact selection and invalidation tuple. `(tenant_id, tax_case_id, selected_at, selected_by)` — selection timestamp/actor tuple for audit.
- **Foreign keys:** `(tenant_id, tax_case_id)` to TaxCase; composite FK `(tenant_id, tax_case_id, selected_export_id, selected_export_content_hash, selected_export_validation_outcome, selected_export_integrity_status)` to ExportRun candidate key `UNIQUE(tenant_id, tax_case_id, export_id, content_hash, validation_outcome, integrity_status)` (ensures selected export belongs to the same case and exact output/status). Enforce `CHECK ((binding_status = 'ACTIVE' AND selected_export_validation_outcome = 'PASSED' AND selected_export_integrity_status = 'CURRENT') OR (binding_status = 'STALE' AND selected_export_integrity_status = 'STALE') OR (binding_status = 'DRIFTED' AND selected_export_integrity_status = 'DRIFTED'))`; a stale/drifted binding is retained only as an invalidated historical binding and cannot be used for an attempt.
- **Row Creation:** A binding row is created and inserted only when an export is selected for the first time. Before selection, no row exists. The INSERT is atomic: the complete selected-export identity (`selected_export_id`, `selected_export_content_hash`, `selected_export_validation_outcome = 'PASSED'`, `selected_export_integrity_status = 'CURRENT'`), `binding_status = 'ACTIVE'`, `selected_at` (timestamp), `selected_by` (actor), and `locked_at=NULL` are all set in one transaction. All identity and actor fields are NOT NULL on insert.
- **Before First Attempt:** the complete selected-export tuple, `selected_at`, and `selected_by` are mutable via UPDATE with WHERE `locked_at IS NULL`. Each update changes the complete tuple atomically and rechecks the exact ExportRun FK and active/PASSED/CURRENT CHECK. Selection change history records tenant_id, tax_case_id, old/new export identity and status, old/new selected_at/by, actor, and timestamp. Use serialized protocol: SELECT FOR UPDATE (Postgres/MySQL) or writer transaction (SQLite) on binding row, recheck `locked_at IS NULL`, then UPDATE with exactly-one-row check.
- **After First Attempt:** First SubmissionAttempt insert atomically sets `locked_at` to non-null timestamp within same serialized transaction. Row becomes immutable for selection identity; only the atomic invalidation gate may transition `binding_status` and `selected_export_integrity_status` to STALE/DRIFTED. Any attempted selection update after this flag is non-null fails closed.
...
- **First Attempt Requirement:** First attempt requires an existing `ACTIVE` binding row with non-null selected export identity, exact content hash, `selected_export_validation_outcome = 'PASSED'`, and `selected_export_integrity_status = 'CURRENT'`.
- **Gate:** Selection may be changed only via UPDATE before first SubmissionAttempt, updating the complete selected-export tuple atomically. Once first attempt exists and locked_at is non-null, this binding becomes immutable for selection; a stale/drifted binding cannot be reused, exported, or submitted, and subsequent corrections use a successor TaxCase (PT-016) with its own new binding row.

### 2.15 `submission_attempts`

- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `attempt_id` (positive integer, sequence 1, 2, 3, ...), `attempt_number` (CHECK `attempt_number > 0`), `binding_locked_at` (FK link to submission_bindings.locked_at), `selected_export_id`, `selected_export_content_hash`, `selected_export_validation_outcome`, `selected_export_integrity_status`, `binding_status`, `attempted_at` (submission timestamp), `attempted_by` (actor), immutable request/output `hash` (e.g., portal request/response hash), `outcome` (SUBMITTED, RECEIVED, ACCEPTED, REJECTED, UNKNOWN, or similar immutable outcome label). Nullable: `previous_attempt_id`, `previous_attempt_number` (required only for attempt_number > 1).
...
- **Foreign keys:** composite FK `(tenant_id, tax_case_id, binding_locked_at, selected_export_id, selected_export_content_hash, selected_export_validation_outcome, selected_export_integrity_status, binding_status)` to the submission_bindings exact selection candidate (ensures the locked binding still names the exact export tuple and ACTIVE status); composite FK `(tenant_id, tax_case_id, selected_export_id, selected_export_content_hash, selected_export_validation_outcome)` to ExportRun candidate `UNIQUE(tenant_id, tax_case_id, export_id, content_hash, validation_outcome)`; composite FK `(tenant_id, tax_case_id, previous_attempt_id, previous_attempt_number)` to submission_attempts candidate `(tenant_id, tax_case_id, attempt_id, attempt_number)` (only when both previous fields non-null) creates self-referential chain.
- **Immutability:** tenant_id, tax_case_id, attempt_id, attempt_number, selected export identity/status, binding_status, attempted_at, attempted_by, immutable request/output hash, outcome, portal receipt/status/response, binding_locked_at are immutable after insert. Retries are separate rows with attempt_number incremented.
...
- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `source_id`, `readiness_status`. Also required: source type, source identity and period. Conditional readiness fields: `readiness_evidence_artifact_id`, `readiness_evidence_artifact_hash`, `readiness_evidence_binding_hash`, `readiness_catalog_hash`, `readiness_actor`, `readiness_reason`, `readiness_scope`, and `readiness_at`. Nullable: institution/account metadata, reconciliation state, and readiness fields only where permitted by the conditional CHECK below.
...
- **Readiness integrity:** `readiness_status` is NOT NULL and every transition must use one of the CHECK values; NULL cannot bypass the readiness state machine or a readiness gate. Enforce the conditional CHECK: `(readiness_status = 'READY' AND readiness_evidence_artifact_id IS NOT NULL AND readiness_evidence_artifact_hash IS NOT NULL AND readiness_evidence_binding_hash IS NOT NULL AND readiness_catalog_hash IS NOT NULL AND readiness_actor IS NOT NULL AND readiness_at IS NOT NULL) OR (readiness_status = 'DECLARED_NOT_APPLICABLE' AND readiness_evidence_artifact_id IS NOT NULL AND readiness_evidence_artifact_hash IS NOT NULL AND readiness_evidence_binding_hash IS NOT NULL AND readiness_actor IS NOT NULL AND readiness_reason IS NOT NULL AND readiness_scope IS NOT NULL AND readiness_at IS NOT NULL) OR (readiness_status NOT IN ('READY', 'DECLARED_NOT_APPLICABLE') AND readiness_evidence_artifact_id IS NULL AND readiness_evidence_artifact_hash IS NULL AND readiness_evidence_binding_hash IS NULL AND readiness_catalog_hash IS NULL AND readiness_actor IS NULL AND readiness_reason IS NULL AND readiness_scope IS NULL AND readiness_at IS NULL)`. The evidence tuple is NOT NULL exactly when the status claims READY or DECLARED_NOT_APPLICABLE.
- **Foreign-key readiness binding:** when present, composite FK `(tenant_id, tax_case_id, source_id, readiness_evidence_artifact_id, readiness_evidence_artifact_hash)` references `external_source_artifacts` exact candidate key; `readiness_evidence_binding_hash` is the immutable hash of the ordered exact required evidence bindings, and `readiness_catalog_hash` is the immutable hash of the complete nonempty required catalog. A READY or DECLARED_NOT_APPLICABLE row without that exact source/evidence binding fails closed.
- **Gates:** complete required catalog enumerated before readiness; mandatory unresolved entries block action. Empty catalog cannot become ready. All BookSet membership explicit via external_source_booksets junction; artifact associations explicit via external_source_artifacts junction. READY requires the exact complete-catalog binding; DECLARED_NOT_APPLICABLE requires the exact evidenced justification binding and actor/reason/scope.
...
10. **Correction invalidation:** in the same atomic transaction as a committed correction or late source, deterministically enumerate every affected FilingSnapshot and ExportRun, set each `integrity_status` to `STALE` or `DRIFTED`, and persist non-null `invalidation_reason`, `invalidation_source`, `invalidation_at`, and exact same-tenant `superseding_tax_case_id`. The transaction also revokes any affected active selection binding. Any enumeration, marking, binding-revocation, or successor-link failure rolls back the correction; prior snapshots and exports remain immutable, and every reuse/export/submission gate requires `CURRENT` status and a deliberate regenerated successor artifact.
