...
- **Fields:** NOT NULL: `snapshot_id`, `tenant_id`, `tax_case_id`, `pan`, `period_key`, `filing_sequence`, `eligibility_fact_set_id`, `eligibility_fact_set_hash`, `authority_pack_id`, `authority_pack_content_hash`, `schema_artifact_id`, `schema_artifact_hash`, `governing_act`, `filing_trigger`, `rule_snapshot_id`, `seal_state` (`OPEN` or `SEALED`), `integrity_status` (`CURRENT`, `STALE`, or `DRIFTED`), as_of_instant timestamp, snapshot_content_hash (computed over inputs: ordered BookSet entries + artifact hashes + exact frozen fact-set ID/hash + exact TaxCase pack/rule/schema tuple), declared `bookset_membership_count` (expected nonempty count of filing_snapshot_booksets entries), declared `bookset_membership_hash` (hash of ordered snapshot BookSet IDs/ledger-versions), declared `artifact_membership_count` (expected nonempty count of filing_snapshot_artifacts entries), declared `artifact_membership_hash` (hash of ordered snapshot artifact ID/hash tuples). Nullable until `seal_state = 'SEALED'`: `sealed_at`, `sealed_by`. Nullable only when `integrity_status = CURRENT`: `invalidation_kind`, `invalidation_reason`, `invalidation_source`, `invalidation_at`, `superseding_tax_case_id`, `replacement_snapshot_id`, and `replacement_snapshot_content_hash`.
...
- **Immutability:** snapshot ID, eligibility fact set (ID, hash), authority pack (ID, content_hash, Act, period, trigger, rule), timestamp, and snapshot_content_hash are immutable. A new snapshot is created when books, sources, or facts change before submission. Snapshot content hash explicitly includes exact frozen fact-set ID/hash to prevent fact drift.
- **Seal state:** `seal_state` has `CHECK (seal_state IN ('OPEN', 'SEALED'))`. Before seal, filing_snapshot_booksets and filing_snapshot_artifacts may be inserted. The `OPEN -> SEALED` transition is one-way and enforced by database assertion: both membership sets must be nonempty, both must have non-null membership/artifact counts equal to declared counts, and both hashes must match exactly computed values. After seal, insert/update/delete triggers on snapshot and membership junctions are rejected. Only SEALED snapshots may feed TaxComputation, ExportRun, submission bindings, or submission attempts. A SEALED snapshot cannot become OPEN again.
...
- **No computation FK:** FilingSnapshot does NOT store `tax_computation_id` or declared computation version/hash; this breaks circular dependency. TaxComputation (2.13) is the immutable one-to-one child created atomically with this snapshot, bound by exact SEALED snapshot ID/hash and the full applicability tuple. The computation is created after snapshot structure is frozen, guaranteeing no circular insert.
- **BookSet and Artifact Membership:** Explicit via junction tables filing_snapshot_booksets (2.12a) and filing_snapshot_artifacts (2.12b). Membership must be nonempty before snapshot can seal. Declared membership counts and hashes are verified during seal transition.
- **Gate:** FilingSnapshot must bind exactly one sealed AuthorityPack through the full TaxCase candidate key `(authority_pack_id, authority_pack_content_hash, governing_act, period_key, filing_trigger, rule_snapshot_id, schema_artifact_id, schema_artifact_hash)`, bind exactly one sealed eligibility_fact_set via the exact (ID, hash, pan, period, sequence, Act, trigger, rule) tuple to the same case, and enumerate exact BookSet ledger versions via junction (not as-of alone). Every ExportRun is tied to exactly one FilingSnapshot by exact snapshot ID/hash, and only a `SEALED` snapshot can feed computation, export, selection, or submission. Snapshot and its canonical TaxComputation are created atomically; snapshot must be SEALED before computation is eligible for export. Child columns and target columns are identical in declared order; a caller cannot substitute another pack, schema, rule, or fact set.
...
- **Immutability:** all fields immutable after snapshot seals. Database write guard (insert/update/delete trigger) rejects all mutations after parent filing_snapshot.seal_state becomes SEALED.
- **Gate:** Before snapshot is sealed, membership is deterministically enumerated and frozen. Readiness gate: membership must be nonempty. ledger_version and event_cursor must be captured at snapshot creation and remain immutable; both NOT NULL ensures exact ledger state is frozen. Snapshot seal transition verifies membership_count > 0 and membership_hash matches exactly computed value of ordered (snapshot_id, book_set_id, ledger_version) tuples.
...
- **Immutability:** all fields immutable after snapshot seals. evidence_artifact_hash is NOT NULL and forms exact composite FKs; hash cannot be bypassed or null-substituted. source_id is NOT NULL and immutable. Database write guard (insert/update/delete trigger) rejects all mutations after parent filing_snapshot.seal_state becomes SEALED.
- **Gate:** Artifact membership deterministically enumerated at snapshot creation. Each artifact must be bound to its exact source via the composite FK to external_source_artifacts, preventing weaker pairings. Hash verifies artifact integrity across lifecycle via exact composite FK. Parser_version is required where applicable and immutable. Stronger binding via source-artifact tuple prevents cross-source substitution. Snapshot seal transition verifies artifact_count > 0 and artifact_membership_hash matches exactly computed value of ordered (evidence_artifact_id, evidence_artifact_hash) tuples.

### 2.12c Database-enforced FilingSnapshot seal protocol

The seal contract is enforced by the database, not by an application promise. FilingSnapshot has a NOT NULL `seal_state` with the only forward transition `OPEN -> SEALED`. Direct SQL writes are covered by database triggers (or equivalent guarded stored routines plus triggers):

- A snapshot-seal trigger locks/serializes the snapshot, rejects a non-forward transition, requires both `bookset_membership_count > 0` and `artifact_membership_count > 0`, and rejects `SEALED` unless the snapshot has exactly those many distinct entries in filing_snapshot_booksets and filing_snapshot_artifacts respectively, with hashes matching exactly computed values. Snapshot insert/update/delete triggers reject `seal_state` updates after a snapshot is sealed.
- A booking-set and artifact-entry insert/update/delete trigger rejects all mutations after parent filing_snapshot.seal_state becomes `SEALED`, preventing retroactive membership changes. Entries may be inserted only while the parent snapshot is `OPEN`.
- The TaxComputation creation trigger (§2.13) rejects creation of a computation unless its target filing_snapshot has `seal_state = 'SEALED'`. The ExportRun creation trigger (§2.14) rejects creation unless the target snapshot has `seal_state = 'SEALED'`. The SubmissionBinding creation trigger (§2.14a) rejects binding unless the target snapshot has `seal_state = 'SEALED'`.

Dialect realization is required before implementation authority: SQLite uses `BEGIN IMMEDIATE` for the seal transaction plus `BEFORE` triggers with `RAISE(ABORT, ...)`; PostgreSQL uses row-locking seal triggers and `DEFERRABLE INITIALLY DEFERRED` constraint triggers where multi-statement membership assembly needs commit-time validation; MySQL/InnoDB uses a stored seal procedure plus `BEFORE` write/state triggers, `SELECT ... FOR UPDATE`, and the same count/hash checks in one transaction because MySQL has no deferred constraints. In all three dialects, direct table writes fail closed, sealed snapshots and their membership junctions are immutable, and a concurrent writer cannot add membership after assertion. A dialect without these database guards is not conformant.
...
- **Atomicity and Creation:** TaxComputation is created only after the target FilingSnapshot is SEALED. The database creation trigger verifies `filing_snapshot.seal_state = 'SEALED'` before allowing the insert. TaxComputation captures the snapshot's exact ID/hash and pack/rule/schema/fact tuple. No snapshot-declared computation version/hash match is required; the snapshot does not declare computation identity.
- **Gate:** exactly one TaxComputation derives from one FilingSnapshot via UNIQUE(snapshot_id). Multiple ExportRuns may use the same computation if snapshot has not changed. No mutable post-creation update to either entity. Computation cannot be created for an OPEN or previously-sealed snapshot.
...
- **Fields:** NOT NULL: `tenant_id`, `tax_case_id`, `source_id`, `readiness_status`. Also required: source type, source identity and period. Conditional `READY` fields: `readiness_evidence_set_id`, `readiness_evidence_set_state` (fixed=`SEALED`), `readiness_actor`, and `readiness_at`. Conditional `DECLARED_NOT_APPLICABLE` fields: exact `readiness_declaration_artifact_id`, `readiness_declaration_artifact_hash`, `readiness_actor`, `readiness_reason`, `readiness_scope`, and `readiness_at`. Nullable: institution/account metadata, reconciliation state, and readiness fields only where permitted by the conditional CHECK below.
...
- **Readiness integrity:** `readiness_status` is NOT NULL and every transition must use one of the CHECK values; NULL cannot bypass the readiness state machine or a readiness gate. Enforce the conditional CHECK: `readiness_status = 'READY'` requires the exact evidence-set FK, `readiness_evidence_set_state = 'SEALED'`, plus actor/timestamp; `readiness_status = 'DECLARED_NOT_APPLICABLE'` requires all declaration fields above, and every other status requires every readiness field to be NULL. No summary hash, count, or catalog reference is stored directly in this table; all are derived through the evidence-set FK.
- **Foreign-key readiness binding:** READY uses composite FK `(tenant_id, tax_case_id, source_id, readiness_evidence_set_id, readiness_evidence_set_state)` to the `readiness_evidence_sets` candidate key `UNIQUE(tenant_id, tax_case_id, source_id, evidence_set_id, catalog_snapshot_id, catalog_snapshot_hash, seal_state)` with the identical final `seal_state = 'SEALED'`, and therefore can reference only a `SEALED` set. This single FK provides access to the exact sealed catalog, entry count, and hashes without copying. DECLARED_NOT_APPLICABLE uses `(tenant_id, tax_case_id, source_id, readiness_declaration_artifact_id, readiness_declaration_artifact_hash)` to `external_source_artifacts`. The evidence set exact-FKs every catalog entry and source artifact, so no partial or independently substituted artifact is accepted.
- **Gates:** complete required catalog enumerated before readiness; mandatory unresolved entries block action. Empty catalog cannot become ready. All BookSet membership explicit via external_source_booksets junction; artifact associations explicit via external_source_artifacts junction. A TaxCase `READY` transition is a database-guarded aggregate transition: every required source row must be `READY` and must carry the exact composite FK to a `SEALED` evidence set; no copied status, hash, or count is stored in external_sources — all derive from the single sealed-set FK. DECLARED_NOT_APPLICABLE requires the exact evidenced justification binding and actor/reason/scope.
