...
| MCA-AUDIT-018 | s177 Audit Committee composition/functions are governed by s177 and applicability/classes are governed by Board Powers Rule 6. Rule 4 of the same Chapter only restricts matters handled through meeting modes; it is not the committee-applicability rule. No separate Registrar-filed “Audit Committee Report” is created by these provisions. | [Companies Act s177](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Board Powers Rules, Chapter 12](https://www.mca.gov.in/Ministry/pdf/NCARules_Chapter12.pdf) | **OPEN+BLOCK for applicability profile** | Composition/functions are source-verified; exact Rule 6 company applicability and tenant facts must be frozen before execution. |
| MCA-AUDIT-019 | s129(3) requires CFS where a company has one or more subsidiaries or associate companies. AOC-4 CFS is not optional for an associate-only company. Apply only the current documented Accounts Rule 6 exemptions: qualifying wholly/partly owned subsidiary with member notice/no objection, securities not listed or in process of listing, and an ultimate/intermediate holding company filing compliant CFS. A historical FY2014-15 associate-only exception is not a current rule. | [Companies Act s129(3)](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Accounts Rule 6 amendment](https://www.mca.gov.in/Ministry/pdf/CompaniesAccountsAmendmentRules_28072016.pdf), [Accounts Rules rule 12](https://www.mca.gov.in/Ministry/pdf/Rules_09072015.pdf) | **VERIFIED for trigger; OPEN for exemption profile** | AOC-4 CFS is blocked only while current Rule 6 exemption facts are unresolved; never downgrade associate-only current CFS to optional or import a historical exception into a current FY. |
...
| MCA-AUDIT-025 | For every applicable s139(1) appointment, the fourth proviso and Audit Rules rule 4(2) require notice to the Registrar in ADT-1 within 15 days of the appointment meeting. This notice rule is separate from rotation applicability and is not conditioned on small-company/profile class. | [Companies Act s139(1)](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Audit Rules rule 4(2)](https://www.mca.gov.in/Ministry/pdf/NCARules_Chapter10.pdf) | **VERIFIED** | Create the ADT-1 notice state for every applicable s139(1) appointment; keep only rotation/class rules separately profile-driven. |

## Company classification and audit boundary

Freeze the exact effective G.S.R. 700(E) source snapshot before classification. Then apply s2(85) exclusions: a company is not small if it is public, a holding or subsidiary company, registered under s8, governed by a special Act, or excluded by another applicable statutory rule. Only after exclusions pass may the profile test paid-up capital of **≤ ₹4 crore** and turnover of **≤ ₹40 crore**. An OPC can qualify only if it passes the exclusions and both thresholds. See **MCA-AUDIT-015**.
...
| Subsequent appointment | Applicable AGM appointment/re-appointment under s139(1) → ADT-1 notice to Registrar within 15 days of the appointment meeting → separate term/rotation profile check. | Do not invent a generic 30-day appointment rule or make ADT-1 depend on rotation/small-company class; **MCA-AUDIT-004**, **MCA-AUDIT-025**. |
...
ADT-1 is the prescribed Registrar notice named by Audit Rules rule 4(2) for the fourth proviso to s139(1), and it is due within 15 days of every applicable appointment meeting. This notice state is not a small-company/profile-class gate; auditor rotation applicability remains a separate rule/profile check. See [Companies Act s139(1)](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Audit Rules rule 4(2)](https://www.mca.gov.in/Ministry/pdf/NCARules_Chapter10.pdf), and **MCA-AUDIT-025**.
...
2. The approved statements are signed on behalf of the Board by the chairperson when Board-authorised **or** by two directors, one being the managing director if any, plus the CEO if a director, CFO, and company secretary wherever appointed. For an OPC, one director signs.
3. The signed statements are submitted to the auditor for the auditor report.
4. The auditor report is received and attached to the financial statements.
5. The company proceeds to AGM/member adoption; if adoption fails at the AGM or adjourned AGM, the signed unadopted statements and required attachments proceed through the provisional s137 filing path within 30 days.
...
| AOC-4 CFS | Consolidated financial-statement filing where s129(3) is triggered by one or more subsidiaries or associate companies, subject only to current documented Accounts Rule 6 exemptions. An associate-only company is not optional; the historical FY2014-15 exception is not current. | Trigger verified; current Rule 6 exemption profile and field schema OPEN+BLOCK. | [s129(3)](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Accounts Rule 6](https://www.mca.gov.in/Ministry/pdf/CompaniesAccountsAmendmentRules_28072016.pdf), [rule 12](https://www.mca.gov.in/Ministry/pdf/Rules_09072015.pdf) |
...
| ADT-1 | Registrar notice for every applicable s139(1) appointment, within 15 days of the appointment meeting under the fourth proviso and Audit Rules rule 4(2). Rotation applicability is separate. | Verified; do not make notice depend on small-company/profile class. | [Companies Act s139(1)](https://www.mca.gov.in/content/dam/mca/pdf/CompaniesAct2013.pdf), [Audit Rules rule 4(2)](https://www.mca.gov.in/Ministry/pdf/NCARules_Chapter10.pdf) |
...
| Signature and audit order | Board approval → s134 signatures → submission to auditor → auditor report → attachment → AGM/member adoption or provisional s137 filing. OPC uses one director signature. | Audit is incorrectly completed before Board approval/signature, or the report is detached from the filed statements. |
...
1. **Audit Committee applicability profile (MCA-AUDIT-018):** resolve the current Board Powers Rule 6 company-class test and tenant facts. Chapter 12 Rule 4 is meeting-mode-only and must not be used for committee applicability. Composition/functions may not be executed until this is frozen.
...
3. **AOC-4/AOC-4 CFS field schema and CFS exemption profile (MCA-AUDIT-019):** attach the current official source snapshot and test s129(3)/current Rule 6 facts. One or more associates or subsidiaries trigger CFS; an associate-only company is not optional, and the historical FY2014-15 exception is not current.
...
- [Board Powers Rules, Chapter 12](https://www.mca.gov.in/Ministry/pdf/NCARules_Chapter12.pdf)
...
- [G.S.R. 700(E) small-company thresholds](https://www.mca.gov.in/bin/ebook/dms/getdocument?doc=MTgwNDY3Mzc5&docCategory=Notifications&type=open)
