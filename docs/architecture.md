...
1. **Enumerate affected reports, filing cases, ExportRuns, and audit packs**: Query all generated reports (P&L, balance sheet, aging, reconciliation, compliance exports, custom tags/splits), filing cases/snapshots (GSTR-1, GSTR-3B, income-tax, payroll, TDS/TCS, and other compliance filings) and their ExportRuns, and audit packs that contain or reference the corrected posting(s), source document, filing period, or related aggregate.
2. **Atomically mark STALE/DRIFTED**: For each affected report snapshot, income-tax FilingSnapshot, ExportRun, filing case, or audit pack, set durable integrity fields `integrity_status`, `invalidation_reason`, `invalidation_source`, `invalidation_at`, and exact `superseding_tax_case_id` where the personal-tax contract applies. All enumerated items must be marked in the same transaction; partial marking is not permitted.
3. **Block reuse/export/submission**: While a FilingSnapshot or ExportRun is marked `STALE/DRIFTED`, it cannot be reused, re-exported, selected/bound, or submitted to a portal, and any affected submission binding is revoked. A regenerated filing receives a new snapshot/export identity and must re-compute → validate to exactly `PASSED` → review → re-close where applicable before selection or submission.
4. **Preserve prior artifacts immutable**: Original exported reports, filing cases, FilingSnapshots, ExportRuns, and audit packs remain visible with their original state, evidence binding, timestamp, and `STALE/DRIFTED` marker. Users/auditors can see the prior artifact, its marker, and link to the correction that invalidated it.
