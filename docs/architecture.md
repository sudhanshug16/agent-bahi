...
1. **Classify and inspect actual submission history first**: For each affected personal-tax artifact, serialize the TaxCase and its immutable SubmissionBinding/event stream, then query actual `SubmissionAttempt` rows. The result selects exactly one kind: `PRE_SUBMISSION_REPLACEMENT` when zero attempts exist, or `POST_SUBMISSION_SUCCESSOR` when at least one attempt exists. A copied status label cannot satisfy this assertion.
2. **Create the replacement identity before invalidation**: For `PRE_SUBMISSION_REPLACEMENT`, create a different `CURRENT` FilingSnapshot, ExportRun, immutable SubmissionBinding, and initial `ACTIVE` binding-state event in the same TaxCase. For `POST_SUBMISSION_SUCCESSOR`, create a different TaxCase, an exact `tax_case_lineage` successor with sequence +1 and matching PAN, period, and filing trigger, then create its independent current snapshot/export/binding/event stream. Same-case post-submission replacement and self replacement are forbidden; pre-submission replacement must bind different snapshot/export identities.
3. **Atomically mark STALE/DRIFTED**: For each affected report snapshot, income-tax FilingSnapshot, ExportRun, filing case, or audit pack, set durable integrity fields `integrity_status`, typed `invalidation_kind`, `invalidation_reason`, `invalidation_source`, `invalidation_at`, exact `superseding_tax_case_id`, and exact replacement snapshot/export IDs and hashes where the personal-tax contract applies. Conditional checks and deferred assertions require non-null fields, exact FKs, actual attempt existence, and the appropriate same-case or successor lineage. All enumerated items must be marked in the same transaction; partial marking is not permitted.
4. **Block reuse/export/submission through append-only binding events**: While a FilingSnapshot or ExportRun is marked `STALE/DRIFTED`, it cannot be reused, re-exported, selected/bound, or submitted to a portal. Append `STALE`/`DRIFTED` to each affected SubmissionBinding's state-event stream; do not mutate its selection identity or include mutable state in a historical SubmissionAttempt FK. A regenerated filing receives a new snapshot/export/binding identity and must re-compute → validate to exactly `PASSED` → review → re-close where applicable before selection or submission.
5. **Preserve prior artifacts immutable**: Original exported reports, filing cases, FilingSnapshots, ExportRuns, SubmissionBindings, binding-state events, attempts, and audit packs remain visible with their original state, evidence binding, timestamp, and `STALE/DRIFTED` marker. Users/auditors can see the prior artifact, its marker, and link to the correction that invalidated it.
6. **Atomic failure rolls back correction**: If enumeration, actual-attempt inspection, replacement creation, lineage/FK validation, deferred assertion, marking, or binding-event append fails, the entire correction transaction rolls back. No partial/silent state; all-or-nothing invalidation or no correction. This preserves rollback/no-reuse semantics matching the global rule and Branch A.

---

### Personal-tax identity and evidence-set boundary

The personal-tax relational contract is closed through one exact identity chain:

`TaxCase(AuthorityPack ID/hash + applicability/rule/schema) → FilingSnapshot(exact same tuple) → TaxComputation → ExportRun(exact snapshot ID/hash + output hash) → SubmissionBinding(immutable binding ID + exact snapshot/export ID/hash) → SubmissionAttempt(single composite binding FK)`.

No downstream row may supply an independently substitutable AuthorityPack, schema, snapshot, or export tuple. SubmissionBinding selection identity is immutable; current validity and invalidation live only in append-only binding-state events, so later invalidation cannot break or rewrite historical attempts. Personal-tax readiness uses sealed catalog snapshots with exact ordinal/`requirement_key` entries and normalized one-to-one evidence-set entries. Each evidence entry exact-FKs both its catalog entry and source artifact, and deferred count/set-equality assertions are required before `READY`; a summary hash or partial set is insufficient.
...
- **CLI-001** through **CLI-007** (CLI contract). **CLI-008** is owner-approved
  as the global semantic contract; its implementation, Gate0/phase
  authorization, and PT-014 details remain separately pending.
...
- [Personal Tax Physical-Schema RFC](discovery/personal-tax-physical-schema.md): Tentative dialect-neutral relational contract, ownership rules, transaction gates, and multi-dialect Gate0 proof obligations.
